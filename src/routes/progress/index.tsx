...
function ProgressPage() {
  ...
  if (!user) return (
    <div className="mx-auto max-w-md rounded-2xl border border-border bg-card p-6 text-center">
      ...
      <Link to="/profile" className="mt-3 inline-block rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground">Sign in</Link>
    </div>
  );
  ...
  const workoutStreak = (() => {
    ...
    for (let i = sorted.length - 1; i >= 0; i--) {
      if (sorted[i].workout_completed) s++; else break;
    }
    return s;
  })();
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await upsert({
        ...
        calories_consumed: form.calories_consumed != null && form.calories_consumed !== ("" as any) ? Number(form.calories_consumed) : null,
        protein_consumed: form.protein_consumed != null && form.protein_consumed !== ("" as any) ? Number(form.protein_consumed) : null,
        water_liters: form.water_liters != null && form.water_liters !== ("" as any) ? Number(form.water_liters) : null,
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <Stat icon={Scale} label="Latest weight" value={latest?.weight_kg ? `${latest.weight_kg} kg` : "—"} />
        <Stat icon={Flame} label="Calories today" value={todayLog?.calories_consumed ? `${todayLog.calories_consumed}${targets ? ` / ${targets.calories}` : ""}` : "—"} />
        <Stat icon={Target} label="Protein today" value={todayLog?.protein_consumed ? `${todayLog.protein_consumed}${targets ? ` / ${targets.protein}g` : "g"}` : "—"} />
        <Stat icon={Dumbbell} label="Workout streak" value={`${workoutStreak} day${workoutStreak === 1 ? "" : "s"}`} />
        <Stat icon={Activity} label="Meals followed" value={`${mealCount}`} />
        <Stat icon={Droplet} label="Water today" value={todayLog?.water_liters ? `${todayLog.water_liters} L` : "—"} />
      </div>
      ...
      {/* Charts */}
      {sorted.length === 0 ? (
        <EmptyState icon={Activity} title="No logs yet" message="Log today to start tracking your progress." />
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          <Chart title="Weight trend">
            <LineChart data={sorted}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="weight_kg" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 3 }} connectNulls />
            </LineChart>
          </Chart>
          <Chart title="Protein intake (g)">
            <LineChart data={sorted}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="protein_consumed" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 3 }} connectNulls />
            </LineChart>
          </Chart>
          <Chart title="Calories intake">
            <LineChart data={sorted}>
              ...
              <YAxis tick={{ fontSize: 10 }} />
              <Tooltip />
              <Line type="monotone" dataKey="calories_consumed" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 3 }} connectNulls />
            </LineChart>
          </Chart>
          <Chart title="Workouts last 7 days">
            <BarChart data={last7.map((l) => ({ ...l, w: l.workout_completed ? 1 : 0 }))}>
              ...
              <Tooltip />
              <Bar dataKey="w" fill="hsl(var(--primary))" />
            </BarChart>
          </Chart>
        </div>
      )}
      ...
      <p className="text-center text-xs text-muted-foreground">
        GymSathi provides general fitness and nutrition guidance only. It is not medical advice. Consult a professional for medical conditions, injuries, or special diets.
      </p>
    </div>
  );
}
