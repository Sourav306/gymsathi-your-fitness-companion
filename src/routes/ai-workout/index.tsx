...
function Page() {
  ...
  const markDayDone = async (dayIdx: number) => {
    ...
    if (dayIdx === 0 && next.has(0)) {
      ...
      if (error) toast.error("Couldn't update progress");
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Built around your gym access, experience and limitations.
        </p>
      </header>
      ...
      {!plan && (
        <div className="rounded-3xl border border-dashed border-border bg-card/50 p-8 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            Build a multi-day plan with sets, reps, form tips and rest times.
          </p>
          <button
            onClick={run}
            disabled={busy}
              ...
              return (
                <div key={i} className="overflow-hidden rounded-2xl border border-border bg-card">
                  <div className="flex items-center justify-between gap-2 border-b border-border px-4 py-3">
                    ...
                    <button
                      onClick={() => markDayDone(i)}
                      className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs ${dayDone ? "bg-primary text-primary-foreground" : "border border-border hover:bg-accent/40"}`}
                    >
                      <CheckCircle2 className="h-3 w-3" />{" "}
                      {dayDone ? "Completed" : "Mark workout completed"}
                    </button>
                  </div>
                      ...
                      return (
                        <div key={j} className="px-4 py-3">
                          ...
                          <div className="mt-1 text-xs">
                            <span className="font-semibold">Form:</span> {ex.formTip}
                          </div>
                          <div className="text-xs text-destructive/80">
                            <span className="font-semibold">Avoid:</span> {ex.commonMistake}
                          </div>
                          <div className="mt-2 flex flex-wrap gap-2 text-xs">
                            ...
                            <button
                              onClick={() => setSwap({ dayIdx: i, exIdx: j })}
                              className="inline-flex items-center gap-1 rounded-full border border-border px-2 py-1 hover:bg-accent/40"
                            >
      ...
      <section className="space-y-3">
        <h2 className="flex items-center gap-2 font-display text-xl font-bold">
          <BookOpen className="h-5 w-5" /> Saved workout plans
        </h2>
        {saved.loading ? (
      ...
      <p className="text-center text-xs text-muted-foreground">
        General fitness guidance only. Stop and consult a professional if you feel pain.
      </p>
