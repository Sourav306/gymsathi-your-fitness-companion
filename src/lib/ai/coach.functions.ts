...
Rules: 7 days. Respect dietary preference, allergies, disliked foods. Indian-friendly meals. Practical for busy people.`;
    const user = `Build a 7-day meal plan for this user: ${JSON.stringify(profile)}`;
    ...
    return { plan: mockMealPlan(profile), source: "mock" };
  });
...
Rules: respect gym access (full_gym/home/no_equipment), experience, injuries. Include 3-5 days. Beginner-safe form tips.`;
    const user = `Build a workout plan for: ${JSON.stringify(profile)}`;
    const raw = await callGateway(sys, user);
    if (raw) {
      try {
    ...
    return { plan: mockWorkoutPlan(profile), source: "mock" };
  });
