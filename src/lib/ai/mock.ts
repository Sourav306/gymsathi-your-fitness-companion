...
function pickMeals(p: UserProfile) {
  ...
  const blocked = (s: string) => {
    ...
    return (
      (dislikes && dislikes.split(/[,\s]+/).some((w) => w && l.includes(w))) ||
  ...
  const breakfast = noEgg
    ...
    : [
        "3 eggs + 2 multigrain rotis",
        "Egg bhurji + toast",
        "Oats + whey + banana",
        "Paneer paratha + 1 egg",
      ];
  const snacks = [
    "Whey shake + 30g almonds",
    "Greek yogurt + apple",
    "Sprouts + buttermilk",
    "Boiled chana + cucumber",
  ];

  return {
    protein: protein.filter((s) => !blocked(s)),
    breakfast: breakfast.filter((s) => !blocked(s)),
    snacks: snacks.filter((s) => !blocked(s)),
  };
}
...
export function mockMealPlan(p: UserProfile, _rp?: RP): MealPlan {
  ...
  const days = DAYS.map((d, i) => ({
    ...
    meals: [
      {
        name: pool.breakfast[i % pool.breakfast.length] || "Oats + milk",
        type: "breakfast" as const,
        calories: perMeal,
        protein: proteinPerMeal,
        ingredients: ["See instructions"],
        prep: "Cook fresh in the morning.",
      },
      {
        name: pool.protein[i % pool.protein.length] || "Dal + rice",
        type: "lunch" as const,
        calories: perMeal,
        protein: proteinPerMeal + 5,
        ingredients: ["See instructions"],
        prep: "Pre-cook protein and grain in batches.",
      },
      {
        name: pool.snacks[i % pool.snacks.length] || "Yogurt + nuts",
        type: "snack" as const,
        calories: Math.round(perMeal * 0.6),
        protein: Math.round(proteinPerMeal * 0.6),
        ingredients: ["See instructions"],
        prep: "Pre-portion in containers.",
      },
      {
        name: pool.protein[(i + 1) % pool.protein.length] || "Soya + roti",
        type: "dinner" as const,
        calories: perMeal,
        protein: proteinPerMeal + 5,
        ingredients: ["See instructions"],
        prep: "Reheat from prep batch.",
      },
    ],
    ...
    grocery: [
      {
        category: "Protein",
        items: veg
          ? ["Paneer 1kg", "Soya chunks 500g", "Tofu 500g", "Greek yogurt 1kg", "Whey protein"]
          : ["Chicken breast 2kg", "Fish 500g", "Eggs 24", "Paneer 250g", "Whey protein"],
      },
      {
        category: "Grains",
        items: ["Brown rice 2kg", "Multigrain atta 1kg", "Oats 500g", "Quinoa 500g"],
      },
      { category: "Dal & Legumes", items: ["Rajma 500g", "Chana 500g", "Moong dal 500g"] },
      {
        category: "Veggies",
        items: ["Onion 2kg", "Tomato 1kg", "Spinach 500g", "Cucumber 500g", "Coriander"],
      },
      {
        category: "Pantry",
        items: ["Cooking oil", "Spices", "Peanut butter", "Almonds 250g", "Milk 4L"],
      },
    ],
    storage:
      "Cook proteins and grains in batches and store in airtight containers in the fridge for 3 days. Freeze portions for days 4–7.",
    budgetTips: [
  ...
  const fullGym = [
    {
      ...
      durationMin: 45,
      exercises: [
        ex("Machine Chest Press", "Chest"),
        ex("Lat Pulldown", "Back"),
        ex("Shoulder Press Machine", "Shoulders"),
        ex("Bicep Curl", "Arms"),
    ...
    },
    {
      ...
      durationMin: 45,
      exercises: [
        ex("Leg Press", "Legs", 3, "10-12", "90 sec"),
        ex("Leg Curl", "Legs"),
        ex("Leg Extension", "Legs"),
        ex("Calf Raise", "Legs", 3, "12-15", "45 sec"),
      ],
    },
    {
      day: "Thu",
    ...
    },
    {
      ...
      exercises: [
        ex("Seated Row", "Back"),
        ex("Chest Supported Row", "Back"),
        ex("Hammer Curl", "Arms"),
        ex("Plank", "Core", 3, "30 sec", "45 sec"),
      ],
    },
  ];
  const home = [
    {
      ...
      exercises: [
        ex("Push-Up", "Chest"),
        ex("Goblet Squat", "Legs", 3, "12", "60 sec"),
        ex("Plank", "Core", 3, "30 sec", "45 sec"),
        ex("Mountain Climbers", "Core", 3, "30 sec", "45 sec"),
      ],
    },
    {
      day: "Wed",
      ...
      durationMin: 30,
      exercises: [
        ex("Walking Lunges", "Legs", 3, "10/leg", "60 sec"),
        ex("Romanian Deadlift", "Legs"),
        ex("Russian Twist", "Core", 3, "20", "45 sec"),
        ex("Plank", "Core", 3, "30 sec", "45 sec"),
      ],
    },
    {
      ...
      durationMin: 30,
      exercises: [
        ex("Burpee", "Full Body", 4, "10", "45 sec"),
        ex("Push-Up", "Chest"),
        ex("Mountain Climbers", "Core", 3, "30 sec", "45 sec"),
      ],
    },
...
export function mockDailyRecommendation(
  p: UserProfile,
  recipeName: string,
  recipeId?: string,
  rp?: RP,
  ...
  return {
    ...
    recipeId,
    groceryNote:
      "Pick up Greek yogurt, eggs/paneer, oats, and a leafy vegetable to stay on track this week.",
    tip,
  };
}
