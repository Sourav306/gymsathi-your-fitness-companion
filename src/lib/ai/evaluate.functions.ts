...
const InputSchema = z.object({
  ...
  tasksLast7: z.array(z.object({ date: z.string(), total: z.number(), completed: z.number() })).optional().default([]),
  metrics: z.object({
...
function deterministicFeedback(input: Input): Feedback {
  ...
  const yPct = input.tasksYesterday.length > 0
  ...
  const avgPct = last7.length > 0
    ? Math.round(last7.reduce((a, d) => a + (d.total > 0 ? (d.completed / d.total) * 100 : 0), 0) / last7.length)
    : null;

  const m = input.metrics || {};
  const status = (actual?: number | null, target?: number | null): "under" | "on_target" | "over" | "unknown" => {
  ...
  const tips: string[] = [];
  if (proteinS === "under") tips.push("Add a high-protein snack: paneer, Greek yogurt, eggs, or a whey shake.");
  if (waterS === "under") tips.push("Keep a water bottle visible to hit your hydration goal earlier in the day.");
  if (workoutS === "missed") tips.push("Try a 20-minute session tomorrow — even short workouts compound.");
  if (stepsS === "under") tips.push("Take a 10-minute walk after each meal to boost steps painlessly.");
  ...
  const msg = `Nice work, ${name}. You completed ${todayCompleted} of ${todayTotal} tasks today (${todayPct}%). ${cmpY}. ${proteinS === "under" ? "Protein was below target — easy fix tomorrow." : proteinS === "on_target" ? "Protein was on target." : ""} ${workoutS === "done" ? "Workout: done ✓" : workoutS === "missed" ? "No workout logged today." : ""}`.replace(/\s+/g, " ").trim();
