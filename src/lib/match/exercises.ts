...
function equipmentAllowed(profile: UserProfile, e: Exercise): boolean {
  ...
  if (profile.gym_access === "home")
    return eq.includes("bodyweight") || eq.includes("dumbbell") || eq.includes("band");
  return true;
}
...
function safeForInjuries(profile: UserProfile, e: Exercise): boolean {
  ...
  for (const m of map)
    if (inj.includes(m.kw) && m.blocks.some((b) => text.includes(b))) return false;
  return true;
}
...
export function suggestExerciseAlternatives(
  ...
  const ranked = pool
    .map((e) => {
      ...
      const order: Record<Difficulty, number> = { Beginner: 1, Intermediate: 2, Advanced: 3 };
      const want =
        profile.experience === "beginner" ? 1 : profile.experience === "intermediate" ? 2 : 3;
